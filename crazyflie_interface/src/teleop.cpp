...
#include "crazyflie_interface/teleop.h"
...
TeleopNode::TeleopNode(): Node("teleop")
{
    subscription_ = this->create_subscription<sensor_msgs::msg::Joy>("joy", 1, 
                                std::bind(&TeleopNode::joyChanged, this, _1));

    pub_cmd_vel_legacy_ = this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel_legacy", 10);
    pub_cmd_full_state_ = this->create_publisher<crazyflie_msgs::msg::FullState>("cmd_full_state", 10);
    ...
    client_emergency_ = this->create_client<std_srvs::srv::Empty>("emergency");
    client_arm_ = this->create_client<crazyflie_msgs::srv::Arm>("arm");
    client_takeoff_ = this->create_client<crazyflie_msgs::srv::Takeoff>("takeoff");
    client_land_ = this->create_client<crazyflie_msgs::srv::Land>("land");
    client_notify_setpoints_stop_ = this->create_client<crazyflie_msgs::srv::NotifySetpointsStop>("notify_setpoints_stop");
}
...
float TeleopNode::angle_normalize(float a)
{ 
    a = fmod(a, 2*M_PI);
    a = fmod((a + 2*M_PI),2*M_PI);
    if (a > M_PI)
...
void TeleopNode::on_parameter_event(const rcl_interfaces::msg::ParameterEvent &event)
{
...
void TeleopNode::joyChanged(const sensor_msgs::msg::Joy::SharedPtr msg)
{
...
sensor_msgs::msg::Joy::_axes_type::value_type TeleopNode::getAxis(
                const sensor_msgs::msg::Joy::SharedPtr &msg, Axis a)
{
    if (a.axis == 0) 
    {
        return 0;
...
void TeleopNode::arm()
{
    ...
    auto request = std::make_shared<crazyflie_msgs::srv::Arm::Request>();
...
void TeleopNode::takeoff()
{
    ...
    auto request = std::make_shared<crazyflie_msgs::srv::Takeoff::Request>();
...
void TeleopNode::land()
{
    ...
    {
        auto request1 = std::make_shared<crazyflie_msgs::srv::NotifySetpointsStop::Request>();
    ...
    auto request2 = std::make_shared<crazyflie_msgs::srv::Land::Request>();
...
void TeleopNode::declareAxis(const std::string& name)
{
...
void TeleopNode::getAxis(const std::string& name, Axis& axis)
{
...
void TeleopNode::on_mode_switched()
{
    if (mode_ == "high_level") 
    {
        // nothing to do
